...
test('Reto 2 - Locator con filter: encontrar producto y leer precio', async ({ page }) => {
  ...
  console.log(`Precio del Samsung galaxy s6: ${textoPrecio}`);
});
...
test('Reto 3 - Locator por atributo parcial: verificar categorías', async ({ page }) => {
  ...
  await expect(
    categorias.filter({ hasText: 'Phones' })
  ).toBeVisible();

  await expect(
    categorias.filter({ hasText: 'Laptops' })
  ).toBeVisible();

  await expect(
    categorias.filter({ hasText: 'Monitors' })
  ).toBeVisible();
